EESchema Schematic File Version 4
LIBS:TD4_RAM-cache
EELAYER 26 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 46
Title "TD4 RAM module"
Date ""
Rev "v1.0"
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Wire Wire Line
	1350 3400 2150 3400
Wire Wire Line
	1350 3500 2150 3500
Text Label 2150 3400 0    50   ~ 0
RW
Text Label 2150 3500 0    50   ~ 0
~WE
Text Notes 1550 3750 0    100  ~ 0
Address bus
Text Notes 1550 4150 0    100  ~ 0
Data bus
Text Notes 750  3250 0    100  ~ 0
Mode and Write Enable
$Sheet
S 4050 2300 1000 200 
U 5C5DC05E
F0 "Address comparator" 50
F1 "Adr_comp.sch" 50
F2 "Comp" O R 5050 2400 100
F3 "Adr" I L 4050 2400 100
$EndSheet
$Sheet
S 5550 2300 650  300 
U 5C5DC063
F0 "Bank selection" 50
F1 "Bank_sel.sch" 50
F2 "Comp" I L 5550 2400 50 
F3 "OE" O R 6200 2400 50 
$EndSheet
Wire Bus Line
	4050 2400 3400 2400
Entry Wire Line
	3300 1800 3400 1900
Entry Wire Line
	3300 1900 3400 2000
Entry Wire Line
	3300 2000 3400 2100
Entry Wire Line
	3300 2100 3400 2200
Wire Wire Line
	3100 1800 3300 1800
Entry Wire Line
	3000 1700 3100 1800
Entry Wire Line
	3000 1800 3100 1900
Entry Wire Line
	3000 1900 3100 2000
Entry Wire Line
	3000 2000 3100 2100
Text Label 3100 1800 0    50   ~ 0
4
Text Label 3100 1900 0    50   ~ 0
5
Text Label 3100 2000 0    50   ~ 0
6
Text Label 3100 2100 0    50   ~ 0
7
Wire Wire Line
	3100 1900 3300 1900
Wire Wire Line
	3100 2000 3300 2000
Wire Wire Line
	3100 2100 3300 2100
Text Label 6600 2500 2    50   ~ 0
~WE
Text Label 6300 3250 0    50   ~ 0
RW
$Comp
L 74xGxx:74AHC1G32 U1
U 1 1 5CF40292
P 6900 2450
F 0 "U1" H 6900 2800 50  0000 C CNN
F 1 "74AHC1G32" H 6900 2700 50  0000 C CNN
F 2 "" H 6900 2450 50  0001 C CNN
F 3 "http://www.ti.com/lit/sg/scyt129e/scyt129e.pdf" H 6900 2450 50  0001 C CNN
	1    6900 2450
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR02
U 1 1 5CF43A7C
P 6900 2550
F 0 "#PWR02" H 6900 2300 50  0001 C CNN
F 1 "GND" H 6905 2377 50  0001 C CNN
F 2 "" H 6900 2550 50  0001 C CNN
F 3 "" H 6900 2550 50  0001 C CNN
	1    6900 2550
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR01
U 1 1 5CF43ADE
P 6900 2350
F 0 "#PWR01" H 6900 2200 50  0001 C CNN
F 1 "VCC" H 6917 2523 50  0001 C CNN
F 2 "" H 6900 2350 50  0001 C CNN
F 3 "" H 6900 2350 50  0001 C CNN
	1    6900 2350
	1    0    0    -1  
$EndComp
$Comp
L power:PWR_FLAG #FLG01
U 1 1 5C704841
P 10200 6300
F 0 "#FLG01" H 10200 6375 50  0001 C CNN
F 1 "PWR_FLAG" H 10200 6473 50  0000 C CNN
F 2 "" H 10200 6300 50  0001 C CNN
F 3 "~" H 10200 6300 50  0001 C CNN
	1    10200 6300
	-1   0    0    1   
$EndComp
$Comp
L power:PWR_FLAG #FLG02
U 1 1 5C704857
P 10500 6300
F 0 "#FLG02" H 10500 6375 50  0001 C CNN
F 1 "PWR_FLAG" H 10500 6474 50  0000 C CNN
F 2 "" H 10500 6300 50  0001 C CNN
F 3 "~" H 10500 6300 50  0001 C CNN
	1    10500 6300
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR03
U 1 1 5C7048B6
P 10200 6300
F 0 "#PWR03" H 10200 6150 50  0001 C CNN
F 1 "VCC" H 10217 6473 50  0000 C CNN
F 2 "" H 10200 6300 50  0001 C CNN
F 3 "" H 10200 6300 50  0001 C CNN
	1    10200 6300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR04
U 1 1 5C704905
P 10500 6300
F 0 "#PWR04" H 10500 6050 50  0001 C CNN
F 1 "GND" H 10505 6127 50  0000 C CNN
F 2 "" H 10500 6300 50  0001 C CNN
F 3 "" H 10500 6300 50  0001 C CNN
	1    10500 6300
	1    0    0    -1  
$EndComp
$Sheet
S 5000 3100 950  750 
U 5C5CC7FC
F0 "Memory block" 100
F1 "Memory_block.sch" 100
F2 "RW" I R 5950 3250 100
F3 "~CS" I R 5950 3400 100
F4 "Data_in" I L 5000 3550 100
F5 "Adr" I L 5000 3250 100
F6 "Data_out" O R 5950 3700 100
$EndSheet
Wire Bus Line
	2600 2400 3000 2400
Wire Bus Line
	2600 2400 2600 3900
Wire Bus Line
	2600 3900 1700 3900
Text Label 1850 3900 0    50   ~ 0
A[0..7]
Wire Bus Line
	1700 4300 4750 4300
Wire Bus Line
	4750 4300 4750 3550
Wire Bus Line
	4750 3550 5000 3550
Text Label 1850 4300 0    50   ~ 0
D[0..4]
Wire Bus Line
	5050 2400 5550 2400
Text Label 3750 2400 2    50   ~ 0
t[0..3]
Text Label 3300 1800 2    50   ~ 0
t0
Text Label 3300 1900 2    50   ~ 0
t1
Text Label 3300 2000 2    50   ~ 0
t2
Text Label 3300 2100 2    50   ~ 0
t3
Wire Bus Line
	2600 3900 4400 3900
Wire Bus Line
	4400 3900 4400 3250
Wire Bus Line
	4400 3250 5000 3250
Connection ~ 2600 3900
Wire Bus Line
	4750 4300 6800 4300
Wire Bus Line
	6800 4300 6800 3700
Wire Bus Line
	5950 3700 6800 3700
Connection ~ 4750 4300
Wire Wire Line
	5950 3250 6300 3250
Wire Wire Line
	6200 2400 6600 2400
Wire Wire Line
	7150 2450 7200 2450
Wire Wire Line
	7200 2450 7200 3400
Wire Wire Line
	5950 3400 7200 3400
Wire Bus Line
	3400 1900 3400 2400
Wire Bus Line
	3000 1700 3000 2400
$EndSCHEMATC
